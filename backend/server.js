...
app.post('/api/call', async (req, res) => {
    ...
    try {
        ...
        if (userResponse === 'accepted') {
            ...
            res.status(200).json({ message: `Call rejected by IP: ${ip}` });
        } else {
            res.status(200).json({ message: `Unknown response from IP: ${ip}` });
        }
    } catch (error) {
        console.error(`Error handling call for IP ${ip}:`, error.message);
        res.status(408).json({ error: 'Call response timed out.' });
    }
});
